...
public class ElasticEmailSender : IEmailSender
{
    ...
    public async Task<EmailSendResult> SendAsync(EmailSendMessage message, CancellationToken cancellationToken = default)
    {
        ...
        {
            return EmailSendResult.Fail("Elastic Email API key não configurada. Verifique a seção ElasticEmail no appsettings.");
        }
        ...
        try
        {
            ...
            var payload = new ElasticEmailRequest
            {
                ...
                {
                    From = fromAddress,
                    ReplyTo = !string.IsNullOrWhiteSpace(_settings.ReplyTo) ? _settings.ReplyTo : null,
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                _logger.LogInformation(
                    "Email enviado via Elastic Email para {Recipient}. TransactionId: {TransactionId}",
            ...
            _logger.LogWarning(
                "Elastic Email retornou {StatusCode} para {Recipient}: {Body}",
                (int)response.StatusCode,
                message.RecipientEmail,
                responseBody);

            return EmailSendResult.Fail($"Elastic Email API error {(int)response.StatusCode}: {responseBody}");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Falha ao enviar email via Elastic Email para {Recipient}", message.RecipientEmail);
            return EmailSendResult.Fail(ex.Message);
        }
    }

    // ===== Elastic Email API v4 DTOs =====
