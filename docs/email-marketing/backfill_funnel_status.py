...
cur.execute("""SELECT id, status FROM customers
  WHERE (status=0 AND (email_sent_count>0 OR last_email_sent_at IS NOT NULL))
     OR (status<2 AND id IN (SELECT DISTINCT customer_id FROM email_events WHERE event_type=3 AND customer_id IS NOT NULL))""")
...
# 2. Qualified: clicou (event_type=3) e status<2
cur.execute("""UPDATE customers SET status=2, updated_by='funnel-backfill-20260903', updated_at=SYSUTCDATETIME()
  WHERE status<2 AND id IN (SELECT DISTINCT customer_id FROM email_events WHERE event_type=3 AND customer_id IS NOT NULL)""")
